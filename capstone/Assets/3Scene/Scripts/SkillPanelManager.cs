...
public class SkillPanelManager : MonoBehaviour
{
    ...
    { //초기 버튼 순서 설정 (버튼 & 쿨타임 오류 대응)
        for (int i = 0; i < 3; i++)
        {
            if (skillList[i] == "과녁 크기 감소 : 상대 턴에 상대방의 과녁 크기가 감소한다.") smallTarget.GetComponent<smallTarget>().setCool(i + 1, 0);
            else if (skillList[i] == "과녁 크기 증가 : 턴이 종료될 때까지 과녁의 크기가 증가한다.") bigTarget.GetComponent<bigTarget>().setCool(i + 1, 0);
            else if (skillList[i] == "과녁 움직이기 : 상대 턴에 상대방의 과녁이 좌우로 움직인다.") movingTarget.GetComponent<movingTarget>().setCool(i + 1, 0);
            ...
            else if (skillList[i] == "풍향 제거 : 1턴 동안 풍향의 영향을 받지 않게 된다.") removeWind.GetComponent<removeWind>().setCool(i + 1, 0);
            else if (skillList[i] == "태풍 : 적이 1턴 동안 풍향의 영향을 2배로 받도록 공격한다.") typhoon.GetComponent<typhoon>().setCool(i + 1, 0);
        }
