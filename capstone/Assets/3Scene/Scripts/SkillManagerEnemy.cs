...
public class SkillManagerEnemy : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (gameManager.enemy_try_skill == true)
        {
            ...
            { //하지만 적팀의 턴에서만 스킬이 사용된다
                skillOn();
            }
            gameManager.enemy_try_skill = false;
        }

        //스테이지 바뀜을 감지하여 스킬 1개 변경을 시도
        if (gameManager.enemy_add_skill == true)
        {
            ...
            Debug.Log("스테이지가 올라, 적팀이 스킬 하나를 바꾸게 되었습니다." + "현재 적 스킬 번호: " + skills[0] + ", " + skills[1] + ", " + skills[2]);
    ...
    { //골랐던 스킬중에서 1개를 랜덤으로 고르게 하고(버튼으로), if문으로 알맞은 스킬을 발동 시킨다
        ...
        if (skills[selected] == 1 && press) // 과녁 크기 감소
        {
            ...
            smallTargetEnemy.GetComponent<smallTargetEnemy>().setCool(selected, 5); //쿨타임 설정
            press = false;
        }
        ...
        if (skills[selected] == 4 && press)
        {
            ...
            reduceCoolEnemy.GetComponent<reduceCoolEnemy>().setCool(selected, 5);
            press = false;
        }
