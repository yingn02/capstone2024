...
public class Timer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (gameManager.playerTurn == true && timer == false) { //플레이어의 턴이면, 타이머가 꺼져 있으면 킨다
        ...
        if (gameManager.playerTurn == false) { //플레이어의 턴이 지나가면, 무조건 타이머를 끈다
        ...
        if (currentTime > 0 && timer == true) //타이머 전원이 켜져 있을 때만 동작함
        {
            ...
            {
                timer = false; //타이머 전원 끄기
                gameManager.Invoke("calculateScore", 0.0f); //무효 처리 (0점)
                currentTime = 0;
            }
