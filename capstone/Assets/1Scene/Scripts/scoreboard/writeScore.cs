...
public class writeScore : MonoBehaviour
{
    ...
    public TextMeshProUGUI cStage; //현재 스테이지
    ...
    public void write_turn(bool player)
    { //누구의 턴인지 점수판에 표시해준다
        if (player)
        {
    ...
    public void write_set(int set, int limit)
    { //새로운 세트가 시작되면 현재 세트가 몇번째 세트인지 표시한다
    ...
    public void write_stage(int stage)
    {
        cStage.text = "" + stage;
    }

    public void clear_score()
    { //세트와 세트점수를 제외한 점수를 초기화
        p1.text = ""; p2.text = ""; p3.text = ""; e1.text = ""; e2.text = ""; e3.text = ""; pTotal.text = ""; eTotal.text = ""; //점수판 초기화
    }

    public void view_score(bool answer)
    { //세트와 세트점수를 제외한 점수를 공개할까요?, 세트 변경 때 잠시 숨기기 위해 만든 함수
    ...
    public void write_set_score(int p_set_score, int e_set_score)
    { //세트 점수 점수판, 새 스테이지일 때만 초기화 할 것
        pSetScore.text = p_set_score.ToString();
        eSetScore.text = e_set_score.ToString();
    }

    public void clear_set_score()
    { //새 스테이지가 되면 세트 점수를 초기화
        pSetScore.text = ""; eSetScore.text = ""; //세트 점수 초기화
    }
